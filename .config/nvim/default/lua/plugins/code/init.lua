        ...
        registry.update(vim.schedule_wrap(function(success, updated_registries)
          if success then
            ...
            if vim.tbl_count(pkg_names) == 0 then
              notif.notify(
                ("Successfully updated %d %s."):format(count, count == 1 and "registry" or "registries"),
                { title = "Mason" }
              )
            end
                ...
                if not pkg:is_installed() then
                  if auto_install then
                    ...
                    notif.notify(
                      ...
                      { title = "Mason", level = vim.log.levels.WARN }
                    )
                  end
                else
                  pkg:check_new_version(function(update_available, version)
                    if update_available then
                      ...
                      pkg
                        :install()
                        :on("closed", function() notif.notify(("Updated %s"):format(pkg.name), { title = "Mason" }) end)
                    else
                ...
                pkg:check_new_version(function(update_available, version)
                  if update_available then
                    ...
                    pkg:install():on("closed", function()
                      running = running - 1
                      if running == 0 then notif.notify "Update Complete" end
                    end)
                  else
                    running = running - 1
                    if running == 0 then
