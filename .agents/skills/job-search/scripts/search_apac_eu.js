...
function matchesProfile(title, company, description, location) {
  ...
  // Check Visa & Location requirements for locations outside Indonesia (all here)
...
async function main() {
  ...
  const searchCachePath = import.meta.dir + "/../cache/search_results_apac_eu_today.json";
  ...
  let md = `# LinkedIn Job Search Results (APAC & Europe, Last 24 Hours, Full-time, 4+ Years Experience)\n\n`;
  md += `Search run at: ${new Date().toISOString()}\n\n`;
  if (matchedJobs.length === 0) {
    md += `No matching jobs found in the last 24 hours in the APAC & Europe regions.\n`;
  } else {
  ...
  await Bun.write(import.meta.dir + "/../job_search_results_apac_eu.md", md);
  console.log("Results written to skills/job-search/job_search_results_apac_eu.md");
}
