...
1. **If no change name provided, prompt for selection**
   ...
   Run `openspec list --json` to get available changes sorted by most recently modified. Then prompt the user to select which change to update.
...
4. **Read and reconcile**
   ...
   - Revise only files that already exist (`existingOutputPaths`). Do NOT create artifacts that don't exist yet, and do NOT invent new files under a glob artifact - note them and advise creating them (e.g., with `/opsx-propose` or `openspec instructions`).
...
6. **Point to the next step (guidance only - NEVER act on it)**
   - Artifacts still missing -> suggest completing them with `/opsx-propose` or `openspec instructions`.
...
- Anything deferred (not-yet-created artifacts or files)
